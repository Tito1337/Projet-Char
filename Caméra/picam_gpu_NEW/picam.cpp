   ...
   while(true) {
      ...
      if(ch != ERR) {
         if(ch == 'q')
            ...
            case 't': // Select next texture
               selected_texture = (selected_texture+1)%(TEXTURE_GRID_ROWS*TEXTURE_GRID_ROWS);
               break;
            case 's': // Select previous texture
               selected_texture = -1;
               break;
            case 'd': // Activate / deactive pipeline (TITO: ??)
            ...
            case 'M':{ // Select next program mode
               setmode = setmode++;
            ...
            // Select current editable RGB component
            case 'r': {
               currcomp = 0;
               break;
            }
            ...
            // Encode replacement color of detected pixels
            case 'c': {
               getnstr(fromkeyb, 49);
            ...
            // Increment / decrement minimum detection value
            case 'i': {
                 currminmax = 0;
            ...
            // Increment / decrement maximum detection value
            case 'a': {
               currminmax = 1;
            ...
            // Increment / decrement detection value 10 times faster
            case '>': {
               if (currminmax) {
