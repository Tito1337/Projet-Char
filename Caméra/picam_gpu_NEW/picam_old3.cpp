...
void object_robot_position (int rtext, int ctext, XYB_TB centroid) // Jérémy Bartholomeus			Fonction calculant la position de l'objet détecté depuis le plan de la caméra vers le plan du robot
{
		...
		float Width = 0.0095;			// taille pixel
		float Height = 0.0106;
		float H = 0;						// hauteur des pièces
		float F = 3.6;						//distance focale
		float k2 = 0;						// distortion
		int 	cmax = 384; 			// nbr pixel
		int	rmax = 256;
		int	dc0 = 0;					//règlage centre caméra
		int	dr0 = 0;
		float	ABC1 = -0.0013;		// coord plan // a la table
		float	ABC2 = 1.2801*0.001;
		float	ABC3 = 0.0015;
		float	m11 =  -0.3321; float	m12 = -0.9204;	float	m13 = -0.2065; float	m14 =  686.3653;  // matrice rotation
		float	m21 =   0.7043; float	m22 = -0.3876; float	m23 =  0.5948; float	m24 = -357.4462;
		float	m31 =  -0.6274; float	m32 =  0.0521; float	m33 =  0.7769; float	m34 =   497.0989;
		float	m41 =   		 0; float m42 = 		    0;	float	m43 =    	   0;	float	m44 =				1;

		float RealC	= (( centroid.Xb  - cmax)/2 ) - dc0;
		float ImgC		= (( centroid.Yb  - rmax)/2 ) - dr0;
		...
		mvprintw(rtext, ctext + 50, "    position objet robot = (%3.1f, %3.1f) ",objro_x, objro_y);
}
