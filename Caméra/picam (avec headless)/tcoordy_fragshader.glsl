...
void main(void) 
{
    float val1 = decode32int(texture2D(tex0, tcoord));
    float val2 = decode32int(texture2D(tex0, tcoord+vec2(1,-1)*texelsize));
    gl_FragColor = encode32int(val1+val2); // convert from float (considered as int32) to rgba
}
